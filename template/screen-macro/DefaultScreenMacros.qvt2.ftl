...
<#macro "container-row">
    ...
    <div class="row<#if .node["@style"]?has_content> ${ec.getResource().expandNoL10n(.node["@style"], "")}</#if>"<#if contRowDivId?has_content> id="${contRowDivId}"</#if>>
        <#list .node["row-col"] as rowColNode>
            ...
            <div class="q-px-xs <#if colHasLg> col-lg-${rowColNode["@lg"]}</#if><#if colHasMd> col-md-${rowColNode["@md"]}<#elseif colHasLg> col-md-12</#if><#if colHasSm> col-sm-${rowColNode["@sm"]}<#elseif colHasLg || colHasMd> col-sm-12</#if><#if rowColNode["@xs"]?has_content> col-${rowColNode["@xs"]}<#elseif colHasLg || colHasMd || colHasSm> col-12</#if><#if rowColNode["@style"]?has_content> ${ec.getResource().expandNoL10n(rowColNode["@style"], "")}</#if>">
                <#recurse rowColNode>
            </div>
        </#list>
    </div>
    ...
    <#else>
        ...
        <#if sri.isAnchorLink(linkNode, urlInstance)>
            ...
            <${linkElement} href="${urlText}"<#if linkFormId?has_content> id="${linkFormId}"</#if><#rt>
                ...
                <#t><#if confirmationMessage?has_content><#if linkElement == "m-link"> :confirmation="'${confirmationMessage?js_string}'"<#else> @click.prevent="moqui.confirmHref($event, '${confirmationMessage?js_string}')"</#if></#if>
        ...
        <${linkFormType} action="${urlInstance.path}" name="${linkFormId!""}"<#if linkFormId?has_content> id="${linkFormId}"</#if><#if linkNode["@target-window"]?has_content> target="${linkNode["@target-window"]}"</#if>><#-- :no-validate="true" -->
            ...
            <#if !linkFormId?has_content>
                ...
                <#if linkNode["image"]?has_content><#assign imageNode = linkNode["image"][0]/>
                    <input type="image" src="${sri.makeUrlByType(imageNode["@url"],imageNode["@url-type"]!"content",null,"true")}"<#if imageNode["@alt"]?has_content> alt="${imageNode["@alt"]}"</#if><#if confirmationMessage?has_content> @click.prevent="moqui.confirmSubmit($event, '${confirmationMessage?js_string}')"</#if>>
                <#else>
                    <#assign iconClass = linkNode["@icon"]!>
                    <#if !iconClass?has_content && linkNode["@text"]?has_content><#assign iconClass = sri.getThemeIconClass(linkNode["@text"])!></#if>
                    <#assign badgeMessage = ec.getResource().expand(linkNode["@badge"]!, "")/>
...
<#macro paginationHeader formListInfo formId isHeaderDialog>
    ...
    <#if (isHeaderDialog || isSavedFinds || isSelectColumns || isPaginated) && hideNav! != "true">
        <div class="form-list-nav-row row q-px-sm q-py-xs">
            ...
            <#if isSavedFinds>
                ...
                <m-container-dialog id="${formId + "_sfdialog"}" title="${savedFormButtonText}">
                    ...
                    <#if activeFormListFind?has_content>
                        ...
                        <#else>
                            <m-form class="form-inline" id="${formId}_SCHED" action="${formSaveFindUrl}" v-slot:default="formProps"
                                ...
                                <q-btn dense outline no-caps type="submit" name="ScheduleFind" @click.prevent="moqui.confirmSubmit($event, '${ec.getL10n().localize("Setup a schedule to send this saved find to you by email?")?js_string}')" label="${ec.getL10n().localize("Schedule")}"></q-btn>
                            </m-form>
                        </#if>
                    </#if>
                    ...
                    <#list userFindInfoList as userFindInfo>
                            ...
                            <div class="big-row">
                                <m-form id="${saveFindFormId}" name="${saveFindFormId}" action="${formSaveFindUrl}" v-slot:default="formProps"
                                    ...
                                    <#if userFindInfo.isByUserId == "true">
                                        <div class="q-my-auto big-row-item"><q-btn dense flat no-caps type="submit" name="DeleteFind" color="negative" icon="delete_forever" @click.prevent="moqui.confirmSubmit($event, '${ec.getL10n().localize("Delete")} ${formListFind.description?js_string}?')"></q-btn></div>
                                    </#if>
                        ...
                        <#else>
                            <div class="big-row">
                                <div class="q-my-auto big-row-item on-left"><q-input dense outlined readonly model-value="${formListFind.description?html}"></q-input></div>
                                <div class="q-my-auto big-row-item"><q-btn dense outline no-caps to="${doFindUrl.pathWithParams}" label="${ec.getL10n().localize("Do Find")}"></q-btn></div>
                                <m-form id="${saveFindFormId}" action="${formSaveFindUrl}" :no-validate="true"
                                    ...
                                    <#if userFindInfo.isByUserId == "true">
                                        <div class="q-my-auto big-row-item"><q-btn dense flat no-caps type="submit" name="DeleteFind" color="negative" icon="delete_forever"
                                                @click.prevent="moqui.confirmSubmit($event, '${ec.getL10n().localize("Delete")} ${formListFind.description?js_string}?')"></q-btn></div>
                                    </#if>
                                    <#if userDefaultFormListFindId == formListFind.formListFindId>
                                        <div class="q-my-auto big-row-item"><q-btn dense outline no-caps type="submit" name="ClearDefault" color="info" label="${ec.getL10n().localize("Clear Default")}"></q-btn></div>
                                    <#else>
        ...
        <#if isHeaderDialog && haveFilters>
        <div class="form-list-span-row q-px-sm q-py-xs">
...
<#else><#-- server rendered, non-static -->
        ...
        <#if !skipHeader>
            <div class="thead">
                ...
                <#if hasSubColumns>
                    ...
                    <div class="tr"><div class="td form-list-sub-row-cell"><div class="form-list-sub-rows"><table class="q-table ${tableStyle}"><thead>
    ...
    <#if listHasContent><#list listObject as listEntry>
        ...
        <#if hasSubColumns><#assign aggregateSubList = listEntry["aggregateSubList"]!><#if aggregateSubList?has_content>
            ...
            <div class="tr"><div class="td form-list-sub-row-cell"><div class="form-list-sub-rows q-table__container q-table__card q-table--horizontal-separator q-table--dense q-table--flat"><table class="q-table ${tableStyle}">
            ...
            </table></div></div><#-- note no closing tr, let following blocks handle it -->
        </#if></#if>
    ...
    <#if !skipEnd>
        ...
        <#-- footer pagination control, outside the CSS table so it can be full-width -->
...
<#macro formListWidget fieldSubNode skipCell isHeaderField isMulti isMultiFinalRow>
    ...
    <#if !isMultiFinalRow && !isHeaderField><#if skipCell><div class="form-group<#if containerStyle?has_content>  ${containerStyle}</#if>"><#else><div class="form-group<#if containerStyle?has_content> ${containerStyle}</#if>"></#if></#if>
    <#t>${sri.pushContext()}
    <#list fieldSubNode?children as widgetNode><#if widgetNode?node_name == "set">${sri.setInContext(widgetNode)}</#if></#list>
    <#list fieldSubNode?children as widgetNode>
        <#if widgetNode?node_name == "link">
    ...
    <#t>${sri.popContext()}
    <#if !isMultiFinalRow && !isHeaderField><#if skipCell></div><#else></div></#if></#if>
</#macro>
...
<#macro fieldRequiredClass subFieldNode><#local vc = formInstance.getFieldValidationClasses(subFieldNode)!><#if vc?contains("required")> required</#if></#macro>
...
<#macro check>
    ...
    <q-field dense outlined<#if fieldLabel?has_content> stack-label label="${fieldLabel}"</#if>
        <#t> class="${containerStyle}<@fieldRequiredClass .node?parent/>"<#if formDisabled!false> disable</#if>
...
<#macro "date-find">
    ...
    <span class="form-date-find">
      <m-date-time id="<@fieldId .node/>_from" name="${curFieldName}_from" type="${.node["@type"]!""}" size="${.node["@size"]!""}"<#rt>
          ...
          <#t><#if fieldsJsName?has_content> v-model="${fieldsJsName}.${curFieldName}_from"<#else> model-value="${fieldValueFrom?html}"</#if>
    ...
    <span class="form-date-find">
      <m-date-time id="<@fieldId .node/>_thru" name="${curFieldName}_thru" type="${.node["@type"]!""}" size="${.node["@size"]!""}"<#rt>
          ...
          <#t><#if fieldsJsName?has_content> v-model="${fieldsJsName}.${curFieldName}_thru"<#else> model-value="${fieldValueThru?html}"</#if>
...
<#macro "date-time">
    ...
    <m-date-time id="<@fieldId .node/>" name="${curName}" type="${.node["@type"]!""}" size="${.node["@size"]!""}" label="<@fieldTitle dtSubFieldNode/>"<#if formDisabled!> disable</#if><#rt>
        <#t><#if fieldsJsName?has_content> v-model="${fieldsJsName}.${curName}"<#else> model-value="${sri.getFieldValueString(dtFieldNode, .node["@default-value"]!"", javaFormat)?html}"</#if>
        <#t> :bg-color="formProps.fieldChanged('${curName}')?($q.dark.isActive?'blue-10':'blue-1'):''"
        <#t><#if .node?parent["@tooltip"]?has_content> tooltip="${ec.getResource().expand(.node?parent["@tooltip"], "")}"</#if>
...
<#macro "drop-down">
    ...
    <m-drop-down name="${name}" id="${tlId}"<#if fieldLabel?has_content> label="${fieldLabel}"</#if><#if formDisabled!> disable</#if><#rt>
            <#t> class="<#if isDynamicOptions>dynamic-options</#if><#if .node["@style"]?has_content> ${ec.getResource().expandNoL10n(.node["@style"], "")}</#if><#if validationClasses?has_content> ${validationClasses}</#if><@fieldRequiredClass ddSubFieldNode/>"<#rt>
            <#t><#if fieldsJsName?has_content> v-model="${fieldsJsName}.${name}" :fields="${fieldsJsName}"<#else><#if allowMultiple> :model-value="[<#list currentValueList as curVal><#if curVal?has_content>'${curVal}',</#if></#list>]"<#else> model-value="${currentValue!}"</#if></#if>
...
<#macro file>
    ...
    <q-file dense outlined<#if fieldLabel?has_content> stack-label label="${fieldLabel}"</#if> class="<@fieldRequiredClass .node?parent/>"<#if fieldsJsName?has_content> v-model="${fieldsJsName}.${curFieldName}"</#if><#if formDisabled!> disable</#if>
...
<#macro password>
    ...
    <q-input dense outlined<#if fieldLabel?has_content> stack-label label="${fieldLabel}"</#if> type="password" name="${curFieldName}" id="<@fieldId .node/>"<#if formDisabled!> disable</#if><#rt>
            ...
            <#t> class="form-control<#if validationClasses?has_content> ${validationClasses}</#if><@fieldRequiredClass .node?parent/>" size="${.node.@size!"25"}"
...
<#macro radio>
    ...
    <#assign curName><@fieldName .node/></#assign>
    <#assign fieldLabel><@fieldTitle .node?parent/></#assign>
    <q-field dense outlined<#if fieldLabel?has_content> stack-label label="${fieldLabel}"</#if> class="<@fieldRequiredClass .node?parent/>"<#if formDisabled!> disable</#if>
        <#t> :bg-color="formProps.fieldChanged('${curName}')?($q.dark.isActive?'blue-10':'blue-1'):''">
        <#if .node?parent["@tooltip"]?has_content><q-tooltip><span v-pre>${ec.getResource().expand(.node?parent["@tooltip"], "")}</span></q-tooltip></#if>
        <template v-slot:control>
        <#list (options.keySet())! as key>
...
<div class="row">
    <q-input dense outlined stack-label label="${fieldLabel} ${ec.getL10n().localize('From')}" name="${curFieldName}_from" id="${tlId}_from"<#if ownerForm?has_content> form="${ownerForm}"</#if><#rt>
            ...
            <#t> :bg-color="formProps.fieldChanged('${curFieldName}_from')?($q.dark.isActive?'blue-10':'blue-1'):''"
            <#t><#if fieldsJsName?has_content> v-model="${fieldsJsName}.${curFieldName}_from"<#else> model-value="${ec.getContext().get(curFieldName + "_from")!?default(.node["@default-value-from"]!"")?html}"</#if>>
    ...
    <q-input class="q-pl-xs" dense outlined stack-label label="${fieldLabel} ${ec.getL10n().localize('Thru')}" name="${curFieldName}_thru" id="${tlId}_thru"<#if ownerForm?has_content> form="${ownerForm}"</#if><#rt>
            <#t> size="${.node.@size!"10"}"<#if .node.@maxlength?has_content> maxlength="${.node.@maxlength}"</#if>
            <#t> :bg-color="formProps.fieldChanged('${curFieldName}_thru')?($q.dark.isActive?'blue-10':'blue-1'):''"
            <#t><#if fieldsJsName?has_content> v-model="${fieldsJsName}.${curFieldName}_thru"<#else> model-value="${ec.getContext().get(curFieldName + "_thru")!?default(.node["@default-value-thru"]!"")?html}"</#if>>
        <#if curTooltip?has_content><q-tooltip><span v-pre>${curTooltip}</span></q-tooltip></#if>
    </q-input>
</div>
</#macro>
...
<#macro "text-area">
    ...
    <#else>
        <q-input type="textarea" dense outlined<#if fieldLabel?has_content> stack-label label="${fieldLabel}"</#if> class="<@fieldRequiredClass .node?parent/>" name="${name}" for="<@fieldId .node/>"
...
<#macro "text-line">
    ...
    <#else>
        ...
        <m-text-line dense outlined<#if fieldLabel?has_content> stack-label label="${fieldLabel}"</#if> id="${tlId}" type="${inputType}"<#rt>
                ...
                <#t> <#if fieldsJsName?has_content>v-model="${fieldsJsName}.${name}" :fields="${fieldsJsName}"<#else><#if fieldValue?html == fieldValue>model-value="${fieldValue}"<#else>:model-value="'${Static["org.moqui.util.WebUtilities"].encodeHtmlJsSafe(fieldValue)}'"</#if></#if>
                ...
                <#t> :bg-color="formProps.fieldChanged('${name}')?($q.dark.isActive?'blue-10':'blue-1'):''"
                <#t> class="<#if validationClasses?has_content>${validationClasses}</#if><@fieldRequiredClass tlSubFieldNode/><#if tlAlign == "center"> text-center<#elseif tlAlign == "right"> text-right</#if>"
                <#t><#if regexpInfo?has_content> pattern="${regexpInfo.regexp}" data-msg-pattern="${regexpInfo.message!"Invalid format"}"</#if>
...
<div class="row">
    ...
    <q-input dense outlined<#if fieldLabel?has_content> stack-label label="${fieldLabel}"</#if> name="${curFieldName}"<#rt>
            ...
            <#t><#if ownerForm?has_content> form="${ownerForm}"</#if>
            <#t><#if fieldsJsName?has_content> v-model="${fieldsJsName}.${curFieldName}"<#else> model-value="${sri.getFieldValueString(.node)?html}"</#if>>
        ...
        <template v-slot:after>
            <#if hideOptions != "operator">
                ...
                <q-select class="on-left" dense outlined options-dense emit-value map-options name="${curFieldName}_op"<#if ownerForm?has_content> form="${ownerForm}"</#if><#rt>
                    ...
                    <#t><#if fieldsJsName?has_content> v-model="${fieldsJsName}.${curFieldName}_op"<#else> model-value="${ec.web.parameters.get(curFieldName + "_op")!defaultOperator!""}"</#if>
